    ...
    return (
        <div className="multi-step-form">
            ...
            <div className="button-group">
                {step === 0 ? (
                    <>
                        <button onClick={() => modalState(false)}>{buttonLabels.cancel}</button>
                        <button onClick={handleNext}>{buttonLabels.next}</button>
                    </>
                ) : (
                    <>
                        <button onClick={handleBack}>{buttonLabels.back}</button>
                        {step < steps.length - 1 ? (
                            <button onClick={handleNext}>{buttonLabels.next}</button>
                        ) : (
                            <button onClick={handleSave}>{buttonLabels.save}</button>
                        )}
                    </>
                )}
            </div>
